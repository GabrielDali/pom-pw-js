...
describe("askQuestion", () => {
  it("resolves with trimmed answer and closes readline interface", async () => {
    const questionSpy = vi.fn((prompt, cb) => cb("  answer  "));
    const closeSpy = vi.fn();
    const createInterfaceSpy = vi.spyOn(readline, "createInterface").mockReturnValue(
      {
    ...
    const result = await askQuestion("What?");
...
describe("isInvalidPageInput", () => {
  ...
  it("returns true when input contains .ts", () => {
    expect(isInvalidPageInput("Dashboard.ts")).toBe(true);
    expect(isInvalidPageInput("page.ts")).toBe(true);
  });

  it("rejects any page name with an extension: only names, no dots (e.g. page.ks, page.py, page.ts, page.js are invalid)", () => {
    expect(isInvalidPageInput("page.js")).toBe(true);
    expect(isInvalidPageInput("page.ts")).toBe(true);
    expect(isInvalidPageInput("page.ks")).toBe(true);
    expect(isInvalidPageInput("page.py")).toBe(true);
    expect(isInvalidPageInput("dashboard.js")).toBe(true);
    expect(isInvalidPageInput("checkout.ts")).toBe(true);
    expect(isInvalidPageInput("Login.ks")).toBe(true);
    expect(isInvalidPageInput("user.py")).toBe(true);
  });
...
describe("findPagesFolder", () => {
  it("returns null when cwd does not exist", () => {
    expect(findPagesFolder("/nonexistent/path/abc123")).toBe(null);
  });

  it("returns null when no pages folder exists", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pom-find-"));
    expect(findPagesFolder(tmp)).toBe(null);
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("finds lowercase pages folder", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pom-find-"));
    fs.mkdirSync(path.join(tmp, "pages"));
    expect(findPagesFolder(tmp)).toBe(path.join(tmp, "pages"));
    fs.rmSync(tmp, { recursive: true, force: true });
  });

  it("finds PascalCase Pages folder", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pom-find-"));
    fs.mkdirSync(path.join(tmp, "Pages"));
    expect(findPagesFolder(tmp)).toBe(path.join(tmp, "Pages"));
    fs.rmSync(tmp, { recursive: true, force: true });
  ...
  it("finds UPPERCASE PAGES folder", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pom-find-"));
    fs.mkdirSync(path.join(tmp, "PAGES"));
    expect(findPagesFolder(tmp)).toBe(path.join(tmp, "PAGES"));
    fs.rmSync(tmp, { recursive: true, force: true });
  ...
  it("does not match a file named pages", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pom-find-"));
    fs.writeFileSync(path.join(tmp, "pages"), "");
    expect(findPagesFolder(tmp)).toBe(null);
    fs.rmSync(tmp, { recursive: true, force: true });
  });
});
