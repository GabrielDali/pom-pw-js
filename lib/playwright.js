...
export function detectLangFromPagesDir(pagesDir) {
  if (!pagesDir || !fs.existsSync(pagesDir)) return null;
  try {
    const entries = fs.readdirSync(pagesDir, { withFileTypes: true });
    const files = entries.filter((e) => e.isFile()).map((e) => e.name);
    const hasTs = files.some((name) => name.endsWith(".ts"));
    const hasJs = files.some((name) => name.endsWith(".js"));
    if (hasTs) return "ts";
    if (hasJs) return "js";
    return null;
  } catch {
