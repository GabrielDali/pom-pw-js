...
export async function runScaffold({ projectName, targetPath, lang, packageRoot }) {
  ...
  // After Playwright's init, collect the devDependencies it installed,
  // then overwrite package.json with our template so main and type are always correct.
  const pkgPath = path.join(targetPath, "package.json");
  let installedDevDeps = {};
  if (fs.existsSync(pkgPath)) {
    try {
      const existing = JSON.parse(fs.readFileSync(pkgPath, "utf-8"));
      installedDevDeps = existing.devDependencies || {};
    } catch {
      // ignore
    }
  }

  const templatePkgPath = path.join(templatesPath, "package.json");
  const templatePkg = JSON.parse(fs.readFileSync(templatePkgPath, "utf-8"));
  templatePkg.name = projectName;
  templatePkg.devDependencies = installedDevDeps;
