...
export async function runScaffold({ projectName, targetPath, lang, packageRoot }) {
  ...
  function ensurePackageJsonMainAndType(targetPath) {
    ...
    try {
      ...
      const out = {
        ...
        main: "index.js",
        scripts: pkg.scripts && Object.keys(pkg.scripts).length > 0 ? pkg.scripts : {},
  ...
  // Prime package.json with correct main and type BEFORE Playwright's init runs.
  // If we don't do this, npm infers main from files in the directory and picks
  // global-setup.js for JS projects, which is wrong.
  const pkgPath = path.join(targetPath, "package.json");
